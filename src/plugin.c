...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Pepper Plugin";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Pepper Plugin";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_author() {
    ...
    return "Thibault Cools & Michaël Deschoolmeester";
}

/* Plugin description */
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "A plugin for showing images in chat.";
}
